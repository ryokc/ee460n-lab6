...
/*Signals generated by MEM stage and needed by previous*/
...
void MEM_stage() {
  ...
  //PSEUDO CODE NOTES:
  //1. w/r memory: enable, we[1:0], address[15:0], data[15:0]
  //2. stall till mem is valid
  //3. target pc <- mem addr, generate trap pc 
...
void AGEX_stage() {
  ...
  //PSEUDO CODE NOTES:
  //1. do operations using NPC, immediate values, sr1, and sr2 as necessary
  //2. latch to mem if [TODO]
  //3. if AGEX.V  gate control instruction signals
...
void DE_stage() {
  ...
  int CONTROL_STORE_ADDRESS = (PS.DE_IR & 0xF800) + (PS.DE_IR & 0x20);
  int ii, jj = 0;
  int LD_AGEX = (dep_stall == 0) && (mem_stall == 0); //TODO: check logic

  //PSEUDO CODE NOTES:
  //1. generate cs addr: control store address <- {IR[15:11], IR[5]}
  //   IR[15:12] = instruction; IR[11] = register/immediate relevant to JSR ; IR[5] = immediate relevant to ADD/AND/XOR
  //2. read registers and cc
  //   SR1 <- DE.IR[8:6]
  //   SR2 <- (DE.IR[13]) ? DE.IR[11:9] : DE.IR[2:0] 
  //3. check dependency
  //   if SR1.NEEDED/SR2.NEEDED and agex,mem,sr have dr of the same value
  //4. latch into AGEX.CS
  //   NPC
  //   latch 20 bits [22:3] from control store
  //   IR
  //   SR1, SR2, CC, DR
  ...
  //reg write TODO: reg write or reg access first?
  if(v_sr_ld_reg) REGS[sr_reg_id] = sr_reg_data;

  //reg access
  int sr1 = (PS.DE_IR & 0x01C0) >> 6; //IR 8:6
  int sr2 = (Get_SR2MUX(CONTROL_STORE[CONTROL_STORE_ADDRESS])) ?
            (PS.DE_IR & 0x0007)
          : (PS.DE_IR & 0x1E00) >> 9; //TODO: is this right
  ...
  if(PS.DE_V){
    ...
    //if agex/mem/sr is writing to needed
    dep_stall = 0;
    if(v_agex_ld_reg && ((sr1_needed && (sr1 == agex_reg_id)) || (sr2_needed && (sr2 == agex_reg_id)))){
      dep_stall = 1;
    }

    if(v_mem_ld_reg && ((sr1_needed && (sr1 == mem_reg_id)) || (sr2_needed && (sr2 == mem_reg_id)))){
      dep_stall = 1;
    }

    if(v_sr_ld_reg && ((sr1_needed && (sr1 == sr_reg_id)) || (sr2_needed && (sr2 == sr_reg_id)))){
      dep_stall = 1;
    }

    //if agex/mem/sr manipulate cc
    if(de_br_op && (v_agex_ld_cc || v_mem_ld_cc || v_sr_ld_cc)){
      dep_stall == 1;
    }
  }
  ...
  if (LD_AGEX) {
    ...
    NEW_PS.AGEX_DRID = (DRMUX) ? 7 : (PS.DE_IR & 0x0E00) >> 9;//DE.IR 11:9
...
void FETCH_stage() {
  ...
  //PSEUDO CODE NOTES:
  //1. access i-cache, wait for assertion in ICACHE_R
  //2. update PC with TARGETPC, TRAPPC, or PC+2
  //3. latch DE if valid, ie. not DEP STALL ; check this // additional DEPSTALL logic?
  //   DE.NPC <- PC+2
  //   DE.IR <- instruction from i-cache
  //   DE.V <- 1
  //   otherwise DE.V <- 0

  //1: update PC
  int ld_pc = (v_de_br_stall) //load pc logic
           && (v_agex_br_stall)
           && (v_mem_br_stall)
           && (icache_r);
  if(ld_pc){ //TODO: update PC logic
    switch(mem_pcmux){
      case 0:
        PC = PC + 2;
      break;
  ...
  //2: access icache with pc
  static int icache_out;
  icache_access(PC, &icache_out, &icache_r);

  //3: latch DE
  int ld_de = (dep_stall == 0)
           && (v_de_br_stall); //load de logic
  if(ld_de = 1 ){
    NEW_PS.DE_NPC = PC+2;
    NEW_PS.DE_IR = icache_out;
    if((dep_stall == 0) && (icache_r == 1)){
      NEW_PS.DE_V = 1;
    }else{
      NEW_PS.DE_V = 0;
    }
